...
void ApplicationMain::RunOneStep(void)
{
	...
	else if(FSKEY_SPACE==key) /// <random number insertion: Question 4.7>
	{
		tree.Insert(rand()%100,0);
	}
	else if(FSKEY_L==key)
	...
	else if(FSKEY_R == key) /// <Right Rotation implementation: Question 4.3>
	{
		tree.RotateRight(mouseOn);
	}
	else if (FSKEY_V == key) /// <Tree-to-Vine implementation: Question 4.6>
	{
		tree.TreeToVine();
	}
	else if (FSKEY_T == key) /// <Vine-to-Tree implementation: Question 4.6>
	{
		tree.VineToTree();
	}
